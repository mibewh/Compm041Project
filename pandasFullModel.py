...
lr = linear_model.LogisticRegression(C=.1, penalty='l1', dual=False) # For select from model feature selection
...
def getpredictionORTB1(pCTR, pCat, budgetRemaining):
    multipliers = {0: 1, 1: 1, 2: 1}
...
for i in range(len(pCTR)):
    bidAmt = getpredictionORTB1(pCTR[i], pCats[i], 25000-spent)
    bid = val_df.iloc[i]
